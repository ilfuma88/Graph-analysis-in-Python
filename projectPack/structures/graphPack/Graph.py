...
from structures.graphPack.Base import Node
from structures.treePack.TreeArrayList import TreeArrayListNode as TreeNode
from structures.treePack.TreeArrayList import TreeArrayList as Tree
from structures.queuePack.Queue import CodaArrayList_deque as Queue
from structures.stackPack.Stack import PilaArrayList as Stack
...
class GraphBase(Graph, metaclass=ABCMeta):
    ...
    def bfs(self, rootId):
        ...
        if rootId not in self.nodes:                #perchè non usare questo anche nella funzione delete node di Graph_AdjancencyList
            return None
        ...
        while not q.isEmpty(): # while there are nodes to explore ...
            ...
            for adj_node in self.getAdj(node):
                if adj_node not in explored:
                    q.enqueue(adj_node)
            if node not in bfs_nodes:           #aggiunto io
    ...
    def dfs(self, rootId):
        ...
        while not s.isEmpty():  # while there are nodes to explore ...
            ...
            for adj_node in self.getAdj(node):
                if adj_node not in explored:
                    s.push(adj_node)
            if node not in dfs_nodes:           #aggiunto io
